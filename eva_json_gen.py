...
def mapping_xml_to_json():
    ...
    for elem in script_node.iter():
        if not(elem.tag in excluded_nodes):
            ...
            if (elem.tag == 'userHandPose'):
               output += ",\n"
               output += user_handpose_process(elem)
...
# # userHandPose node processing #########################################################################
def user_handpose_process(user_handpose_command):
    global gohashid
    user_handpose_node = """    {
         "key": """ + user_handpose_command.attrib["key"] + """,
         "name": "User_Handpose",
         "type": "user_handpose",
         "color": "lightblue",
         "isGroup": false,
